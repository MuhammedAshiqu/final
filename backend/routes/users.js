...
router.post('/editprod',verifySignedIn,(req,res)=>{
  ...
  db.get().collection('products').updateOne({_id:objectId(req.body.id)},
 { $set:{
...
router.get('/describ/:id',function(req,res){

  const id=req.params.id
  console.log("ggg",id)
  db.get().collection('products').findOne({_id:objectId(id)}).then((describ)=>{
    console.log("hh",describ)
    res.json(describ)
    console.log(describ)
