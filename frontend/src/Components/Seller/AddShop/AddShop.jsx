...
function AddShop() {
    ...
    const handleAdd = async () => {
       ...
       await axios.post('https://api.cloudinary.com/v1_1/di0bgblj1/image/upload ', formdata).then((response) => {
        ...
        url && await axios.post('http://localhost:8008/seller/addshop', { input: input,url:url }).then((res) => {
            console.log(res);
    ...
    return (
        <div className="modalBackground">
            <div className="modalContainer">
                ...
                <div className="body">
                    <input type="text" onChange={handleChange} name='name' placeholder='Shop name' />
                    <input type="text" onChange={handleChange} name='category' placeholder='Category' />
                    <input type="number" onChange={handleChange} name='price' placeholder='Contact Number' />
                    <input type="text area"  onChange={handleChange} name='description' placeholder='Description' />
                ...
                <div className="footer">
                    ...
                    <button onClick={handleAdd}>{isloading ? 'confirm':'Add Shop'} </button>
                    <br />
                </div>
            </div>
        </div>
